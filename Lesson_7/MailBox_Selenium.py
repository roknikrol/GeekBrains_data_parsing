...
#                                     'mailbox__control_twostep-short'))
# )
time.sleep(1)
...
while page < 5: # for test purposes - only 5 pages
    emails = driver.find_elements_by_class_name("llc__subject")
    actions = ActionChains(driver)
    for email in emails:
        email.click()
        # button = WebDriverWait(driver, 6).until(email.find_element_by_xpath("//div/h2[@class='thread__subject']"))
        time.sleep(5)
        print(driver.find_element_by_xpath("//div/h2[@class='thread__subject']").text)
